...
test.describe("home", () => {
  test("tirage de 7 dés", async ({ page }) => {
    ...
    const diceButtons = page.locator(".board:nth-child(1) .die");
  ...
  test("déplace dans le premier groupe", async ({ page }) => {
    ...
    await expect(page.locator(".board:nth-child(2) .die")).toHaveCount(1);
  });
  test("déplace dans le deuxième groupe", async ({ page }) => {
    let diceButtons = page.locator(".board:nth-child(1) .die");
    // Clique sur le premier dé pour l’envoyer dans A
    await diceButtons.nth(0).click();
    let ndiceButton = page.locator(".board:nth-child(2) .die");
    // Clique sur le premier dé pour l’envoyer dans B
    await ndiceButton.nth(0).click();
    await expect(page.locator(".board:nth-child(3) .die")).toHaveCount(1);
  });
  ...
  test("visual regression (ignore la première board)", async ({ page }) => {
    ...
    await expect(page).toHaveScreenshot("home-page.png", {
      fullPage: true,
      mask: [page.locator(".board:nth-child(1)")],
